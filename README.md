...
![Is In Stock](screenshots/inventory-service-get.png)

Here is an example where the product is not in stock, we can see that it returns false.

![Not in stock ](screenshots/inventory-service-no-item.png)
...
![Order Placed](screenshots/order-service.png)
...
![Notification](screenshots/notification-service.png)
...
![Product Service](screenshots/test-product-service.png)

#### Order Service

Test result for order service:
![Order Service](screenshots/test-order-service.png)
...
![Order Service Coverage](screenshots/test-coverage-order.png)

##### Inventory Service Coverage

![Inventory Service Coverage](screenshots/test-coverage-inventory.png)

##### Product Service Coverage

![Product Service Coverage](screenshots/test-coverage-product.png)
...
patterns.
I believe microservices are great, however, should be created once a project is already built as a monolithic and when
needed to scale further, slowly migrate to microservices, instead of creating the entire project as a microservice from
the start. I enjoy coding in Java so this project was great to dive deeper out of my comfort zone and create more
complex services that I'm used to. This project exposed me to a few elements of Java / Spring that I haven't been too
familiar with before, such as the Streams API and Web Client Builders.
I found the creation of each service fairly simple to do and understand as most services logic is fairly similar (
Controller, Service, Model, JPA Repository and a DTO).
However, I did find the configuration of the microservices to be somewhat tedious. This is because each service needs
specific application properties, such as JDBC connections, Eureka client configuration, circuit breaker configs (initial
properties as well as timeout and retry properties) and kafka properties. The docker compose file configuration was also
annoying to setup, as I had issues with environments not using the spring profile(an additional application.properties
file) so i had to manually add each conf as JSON in the docker compose file itself. 